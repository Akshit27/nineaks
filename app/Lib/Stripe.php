...
class Stripe
{
	...
	public static function getPaymentMethodsList($customer_id){
		try {
			\Stripe\Stripe::setApiKey(env('STRIPE_SECRET_KEY'));
    ...
    */ 
    ...
    public static function retriveAccount($id){
    	try {
    		\Stripe\Stripe::setApiKey(env('STRIPE_SECRET_KEY'));
    		$account = \Stripe\Account::retrieve($id,[]);
			$response['status'] = 'true';
            $response['data'] = $account;
            $response['message'] = "Account retrived.";
    	} catch (\Exception $e) {
    		$response['status'] = 'false';
            $response['data'] = [];
            $response['message'] = $e->getMessage();
    ...
    public static function createAccountLink($id,$refresh_url,$return_url){
    	try {
    		\Stripe\Stripe::setApiKey(env('STRIPE_SECRET_KEY'));
			...
			$response['status'] = 'true';
            $response['data'] = $account->url;
            $response['message'] = "Account Link.";
    	} catch (\Exception $e) {
    		$response['status'] = 'false';
            $response['data'] = [];
            $response['message'] = $e->getMessage();
